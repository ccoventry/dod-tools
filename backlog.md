...
| **H11** | 🔴 **Hard** | **Process Manager Graceful Shutdown** | Core / Threading | Fix silent channel drop failures (`tx.send().ok()`) in `main.rs` to ensure spawned background processes (`hlae.exe`/`hl.exe`) are explicitly killed if the main GUI application is closed. | Wire up a drop or close handler in the GUI application that propagates a terminate signal or process termination command to all currently active child processes. |
