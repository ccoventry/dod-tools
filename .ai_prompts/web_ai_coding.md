...
2. **Model Recommendation (Meta-Instruction):** *Before* providing the IDE prompt block, explicitly recommend the optimal IDE AI model. You must select from: `Gemini 3.6 Flash (Low/Medium/High)`, `Gemini 3.5 Flash (Low/Medium/High)`, `Gemini 3.1 Pro (Low/High)`, `Claude Sonnet 4.6 (Thinking)`, `Claude Opus 4.6 (Thinking)`, or `GPT-OSS 120B (Medium)`. Default to `Gemini 3.6 Flash (Medium)` for standard file edits. Use `(Low)` for simple text appends (like handoffs), and `(High)` or `Gemini 3.1 Pro` for heavy multi-file refactors. Reserve `Claude Sonnet 4.6 (Thinking)` exclusively for complex architectural shifts.
