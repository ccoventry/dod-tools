...
//!
//! **A copy of `native/src/test_support.rs`, deliberately.** A `#[cfg(test)]`
//! item is not compiled into the library another crate links, so sharing this
//! one would mean giving `native` a `test-support` feature and adding a
//! dev-dependency on it — a build-config change, for thirty lines that have no
//! reason to diverge. If they ever do, that is the moment to make the feature.
//!
