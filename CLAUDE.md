...
- **Pipeline-Owned Commands:** `mirv_movie_filename`, `mirv_recordmovie_start`/`_stop`, `mirv_movie_ffmpeg`, and `host_framerate` are refused outright in both Initial and Scheduled Commands (`native::patch::cfg_scan::BANNED_COMMANDS`, enforced in `map_manager::scan_game_configs`'s report and again independently in `capture_manager::start_capture_batch_impl`) — no setting corresponds to any of them, and a user's own value can misroute captured footage or desync playback with no visible failure. `mirv_movie_fps`/`mirv_movie_separate_hud` are pinned the same way but correspond to a real setting (Output Format tab), so they are shadowed-with-a-warning instead of refused (`cfg_scan::MID_DEMO_HAZARDS`). Adding a new pipeline-internal command that Initial/Scheduled Commands could reach: decide which of these two lists it belongs to before shipping it unprotected.
