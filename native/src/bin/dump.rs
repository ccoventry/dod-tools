...
#[derive(Parser, Debug)]
#[command(version, about = "Dumps a detailed diagnostic summary of a Day of Defeat demo file.")]
struct Args {
...
fn main() {
    ...
    println!("Game Directory:    {}", demo.header.game_directory.to_str().unwrap_or("unknown").trim_end_matches('\x00'));
    ...
    for (entry_idx, entry) in demo.directory.entries.iter().enumerate() {
        println!("Analyzing directory entry {} (title: {:?}, frames: {})", 
            entry_idx, entry.description.to_str().unwrap_or("unknown").trim_end_matches('\x00'), entry.frames.len());
        
        for frame in &entry.frames {
            let debug_name = format!("{:?}", frame.frame_data);
            let frame_type = debug_name.split(|c| c == '(' || c == '{' || c == ' ').next().unwrap_or("Unknown").to_string();
            *frame_counts.entry(frame_type).or_insert(0) += 1;

            match &frame.frame_data {
                FrameData::ConsoleCommand(cmd) => {
                    let cmd_str = cmd.command.to_str().unwrap_or("").trim_end_matches('\x00').trim().to_string();
                    if !cmd_str.is_empty() && !console_commands.contains(&cmd_str) {
                        console_commands.push(cmd_str);
                    }
                }
                FrameData::Sound(sound) => {
                    let snd_str = String::from_utf8_lossy(&sound.sample).trim_end_matches('\x00').trim().to_string();
                            ...
                            for msg in msgs {
                                match msg {
                                    NetMessage::EngineMessage(eng_msg) => {
                                        ...
                                        let eng_type = eng_debug.split(|c| c == '(' || c == '{' || c == ' ').next().unwrap_or("Unknown").to_string();
                                        *engine_message_counts.entry(eng_type).or_insert(0) += 1;
                                    }
                                    NetMessage::UserMessage(usr_msg) => {
                                        let name = String::from_utf8_lossy(&usr_msg.name).trim_end_matches('\x00').to_string();
