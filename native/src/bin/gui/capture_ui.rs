...
use crate::types::{
    ...
    GroupedPlayer, GroupedPlayerStreak, FlatStreak,
};
...
pub fn render_patch_ui(ui: &mut egui::Ui, ctx: &egui::Context, _export_queue: &mut Vec<QueuedStreakExport>) {
    ...
    // 1. Wizard UI & Highlight Discovery Section
    ui.group(|ui| {
        ui.vertical(|ui| {
            ui.heading("📂 Standalone Ingestion & Highlights Wizard");
            ui.add_space(4.0);
            ...
            ui.horizontal(|ui| {
                ui.label("Min Kills:");
                ui.add(egui::DragValue::new(&mut rules.min_kills).range(1..=20));

                ui.add_space(10.0);
                ui.label("Max Gap (sec):");
                ui.add(egui::DragValue::new(&mut rules.max_time_gap).range(1.0..=30.0).speed(0.1));
            });
            ...
            if !queued_demos.is_empty() {
                    ...
                    .show(ui, |ui| {
                        match *grouping_mode {
                            ...
                            QueueGroupingMode::ByPlayer => {
                                // Draw from pre-calculated cache
                            ...
                            QueueGroupingMode::Flat => {
                                // Draw sorted flat list of streaks from pre-calculated cache
                ...
                ui.weak("No discovered highlight streaks. Scan demo files above.");
            }
        });
    });
    ...
    ui.group(|ui| {
        ui.vertical(|ui| {
            ...
            ui.add_space(4.0);
            ui.label("Directly patch demo files using absolute scheduled command injection.");
            ui.add_space(8.0);
            ...
            ui.horizontal(|ui| {
                ...
                if ui.add_enabled(!is_running && !queued_demos.is_empty(), btn).clicked() {
                    ...
                    for demo in queued_demos.iter() {
                        ...
                        // Overriding PatcherConfig tickrate with the scanned demo tickrate so scheduling is precise
...
fn spawn_ingestion_thread(files: Vec<PathBuf>, rules: HighlightRules, ctx: egui::Context) {
    ...
    std::thread::spawn(move || {
        ...
        for file in files {
            if let Ok((tickrate, streaks)) = scan_demo_for_highlights(&file, &rules) {
                ...
                if !selectable.is_empty() {
                    newly_discovered.push(QueuedDemo {
                        path: file,
                        streaks: selectable,
                        tickrate,
                    });
        ...
        let queued_demos_lock = get_queued_demos();
        {
            let mut queued = queued_demos_lock.lock().unwrap();
            for new_demo in newly_discovered {
                if !queued.iter().any(|d| d.path == new_demo.path) {
                    queued.push(new_demo);
                }
            }
