...
#[cfg(target_arch = "wasm32")]
fn build_web_tree(files: &[WebFile]) -> DirNode {
...
#[cfg(not(target_arch = "wasm32"))]
fn get_demo_map_name(path: &std::path::Path) -> String {
    use std::io::Read;
...
#[cfg(not(target_arch = "wasm32"))]
fn get_dir_contents_parallel(path: &std::path::Path) -> Vec<DemoListItem> {
...
#[cfg(not(target_arch = "wasm32"))]
fn scan_dir_async(ctx: Context, tx: mpsc::Sender<GuiMessage>, dir: PathBuf) {
...
#[cfg(not(target_arch = "wasm32"))]
fn get_native_roots() -> Vec<PathBuf> {
...
#[cfg(not(target_arch = "wasm32"))]
fn render_native_dir_node(
    ui: &mut egui::Ui,
    path: &std::path::Path,
    current_dir: Option<&std::path::Path>,
    next_dir: &mut Option<std::path::PathBuf>,
    cache: &mut HashMap<PathBuf, Vec<PathBuf>>,
) {
    let name = path
        .file_name()
