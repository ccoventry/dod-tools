...
- `feature/decal-flush-r-and-d` — decal hygiene between capture clips ([#60](https://github.com/ccoventry/dod-tools/issues/60)). **R&D complete and wired into the batch pipeline (2026-08-25); position tiling and an in-game look are what remain.** The ring sweep was validated in game (2026-08-24), and the engine constant it depended on was measured (2026-08-25): `m_Size` for the small bullet hole is ~4 units, so a position may sit 3 units off a surface outward or 4 inward. Movement *along* a fitted plane is unconstrained, so flush positions are meant to be tiled across a plane rather than harvested one at a time — which is what closes the position shortfall. **Wiring:** `CaptureBlock` now carries `record_start_tick`/`record_stop_tick` (filled in builder.rs's scheduling loop), `PatcherConfig` carries `decal_flush`/`decal_ring_limit` (on by default, ring 256, reachable from the Tauri payload but with no UI toggle yet), `init_commands` pins `r_decals` last at demo load, and the clean runs as a pre-pass inside `StreamPatcher::patch` so its four call sites cannot drift. The injected-cvar frame stays off in the pipeline — it would shift every later frame ordinal by +1 and desync the scheduled capture commands from `frame_counter`. **Remaining:** tiling is *not* implemented yet, so the sweep is still partial and says so in the log (41 of the 68 positions a 256-slot ring needs, on `ktps8w9-gorilla`); choose `ring_limit` from a real capture; and **watch a pipeline capture in game** — nobody has yet, and per the issue that is the only test that can catch this feature's failure modes. Full findings on the issue, including why the `r_decals` cvar approach cannot work.
