## Web AI State
* **Current Goal:** Frontend Migration (Tauri & Vite Integration).
* **Last Evaluated:** `native/src/patch/engine.rs` (StreamPatcher `NetworkMessage` match arm: Active standalone frame injection (`DRC_CMD_INEYE`) verified with dynamic player index extraction).
...
- **Frontend Migration:** IN PROGRESS
  - **Tauri & Vite Integration:** Ongoing migration of the frontend stack from native `egui` to a Tauri + Vite architecture.
